...
from models import build_BoxDETR
from util.utils import clean_state_dict


def get_args_parser():
    parser = argparse.ArgumentParser('Box-DETR', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--modelname', '-m', type=str, required=True, choices=['box_detr'])
...
def build_model_main(args):
    if args.modelname.lower() == 'box_detr':
        model, criterion, postprocessors = build_BoxDETR(args)
    else:
...
def main(args):
    ...
    logger = setup_logger(output=os.path.join(args.output_dir, 'info.txt'), distributed_rank=args.rank, color=False, name="Box-DETR")
