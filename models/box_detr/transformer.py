...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, refbox, pos_embed):
        ...
        num_queries = refbox.shape[0]
        if self.num_patterns == 0:
            tgt = torch.zeros(num_queries, bs, self.d_model, device=refbox.device)
        else:
            tgt = self.patterns.weight[:, None, None, :].repeat(1, self.num_queries, bs, 1).flatten(0, 1) # n_q*n_pat, bs, d_model
            refbox = refbox.repeat(self.num_patterns, 1, 1) # n_q*n_pat, bs, d_model

        hs, references = self.decoder(tgt, memory, memory_key_padding_mask=mask,
                          pos=pos_embed, box_unsigmoid=refbox)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_intermediate=False,
        ...
        self.query_scale = MLP(d_model, d_model, d_model, 2)
        self.point_offset = MLP(d_model, d_model, 2 * nhead, 2)
